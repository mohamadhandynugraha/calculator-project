...
export default {
  [types.APPEND_NUMBER](state, { value }) {
    if (state.append) {
      state.visibleValue = parseFloat(
        state.visibleValue.toString() + value.toString()
      );
    } else {
  ...
  [types.TOGGLE_SIGN](state) {
    if (state.visibleValue === 0) {
      return
    }
    if (state.visibleValue > 0) {
      state.visibleValue = parseFloat(
        `-${state.visibleValue.toString()}`
      );
    } else {
      state.visibleValue = parseFloat(
        state.visibleValue.toString().substring(1)
      );
    }
    if (state.operator) {
      state.secondOperand = state.visibleValue;
